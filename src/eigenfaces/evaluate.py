...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate trained Eigenfaces model on raw dataset by default."
    )
    ...
    parser.add_argument("--model-path", default=root_path("models", "eigenfaces", "trainer_eigenfaces.yml"))
    parser.add_argument("--labels-path", default=root_path("models", "eigenfaces", "labels_eigenfaces.json"))
    parser.add_argument("--cascade-path", default=root_path("haar", "haarcascade_frontalface_default.xml"))
    parser.add_argument(
        "--unknown-threshold",
        type=float,
        default=4000.0,
        help="Eigenfaces confidence threshold. Lower = stricter.",
    )
    ...
    parser.add_argument(
        "--report-json",
        default=root_path("reports", "evaluation", "eigenfaces_eval.json"),
    ...
    parser.add_argument(
        "--threshold-sweep",
        default="1000,2000,3000,4000,5000,7000,9000",
        help="Comma-separated Eigenfaces thresholds for sweep analysis.",
    )
    parser.add_argument(
        "--progress-interval",
        type=int,
...
def main() -> None:
    ...
    print("\n[RESULT] Eigenfaces Offline Evaluation")
    ...
    if args.report_json:
        ...
        if not thresholds:
            thresholds = [1000.0, 2000.0, 3000.0, 4000.0, 5000.0, 7000.0, 9000.0]
        ...
        model_variant = derive_model_variant(args.model_path, args.labels_path, fallback="eigenfaces")
        attach_entity_identity(
            report=report,
            model_family="eigenfaces",
